...
updated = open('nsx-edge-details.csv','w+')

for i in range(len(edge_ids)):
    edge_filename = "nsx-edge-details"+edge_ids[i]+".json"
    ...
    for index, line in enumerate(edge_fw_lines,start=1):
        ...
        elif "<primaryAddress>" in line:
            detail_2 = line[:-1]+","
            detail.append(detail_2)
            #updated.write(detail)
        elif "<subnetPrefixLength>" in line:
            detail_3 = line[:-1]+","
