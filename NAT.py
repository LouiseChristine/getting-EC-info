...
updated = open('NAT.csv','w+')

for i in range(len(edge_ids)):
    edge_filename = "NAT"+edge_ids[i]+".json"
    ...
    for index, line in enumerate(edge_fw_lines,start=1):
        ...
        elif "<ruleTag>" in line:
            detail_2 = line[:-1]+","
            detail.append(detail_2)
        elif "<loggingEnabled>" in line:
            detail_3 = line[:-1]+","
        ...
        elif "<translatedAddress>" in line:
            detail_6 = line[:-1]+","
            detail.append(detail_6)
        elif "<ruleType>" in line:
            detail_7 = line[:-1]+","
        ...
        elif "<originalAddress>" in line:
            detail_10 = line[:-1]+","
            detail.append(detail_10)
        elif "<snatMatchDestinationAddress>" in line:
            detail_11 = line[:-1]+","
